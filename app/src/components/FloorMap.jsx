...
function SeatNode({
  ...
  const displayName = seatDisplayName(seat, member);
  let style;
  if (member) {
 ...
 * mode='edit':
 *   - tool='select': 座席クリックで選択+インライン名前編集、ドラッグで移動、ダブルクリックで座席追加
...
export default function FloorMap({
  ...
  function onDoubleClick(e) {
    if (mode !== 'edit' || flowName != null || tool === 'place') return;
        ...
        : tool === 'place'
          ...
          : tool === 'zone'
            ...
            : tool === 'template'
              ...
              : '席をクリックで名前編集 (Tab で隣の席へ) / ドラッグで移動 / 背景ドラッグで範囲選択 (Shift で追加) / Space+ドラッグ・中ボタンでパン / Cmd/Ctrl+C→V でコピペ / 選択席のハンドルでサイズ・向きを変更 / ダブルクリックで席追加';
